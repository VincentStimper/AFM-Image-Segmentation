...
class FCN4VGG:
    ...
    def __init__(self, vgg16_npy_path=None, load_semantic_net=False):
        ...
        self.var_dict = {}
        self.wd = 5e-4
        self.load_semantic_net = load_semantic_net
    ...
    def build(self, rgb, train=False, num_classes=20, random_init_fc8=False,
        ...
        self.upscore32 = self._upscore_layer(self.fuse_pool2,
                                             shape=tf.shape(bgr),
                                             num_classes=num_classes,
    ...
    def _variable_with_weight_decay(self, shape, stddev, wd, name, decoder=False):
        ...
        if name not in self.data_dict:
            ...
            initializer = tf.constant_initializer(value=self.data_dict[name][0], dtype=tf.float32)
    ...
    def save(self, sess, file_name='./vgg16_trained.npy'):
        """
        ...
        for (name, idx), var in list(self.var_dict.items()):
            ...
            if name not in save_dict:
                save_dict[name] = {}
            save_dict[name][idx] = var_out
        
        np.save(file_name, save_dict)
        
        print('Network parameter saved to %s' % file_name)
